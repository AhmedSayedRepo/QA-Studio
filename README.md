...
- **AI usage & cost tracking** — every AI call's exact token usage (straight from the provider's own response, never estimated) is logged automatically. Every signed-in user can see their own usage by date, provider, and model with an estimated cost, exportable or emailed straight from the app; Admins can additionally pull a whole-organization report across every user.
